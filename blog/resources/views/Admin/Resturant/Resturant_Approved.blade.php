...
<section class="content">
	<form  action="{{url('Search_Restaurant_Approved_or_Hide/')}}" method="get" class="form-submit labels-uppercase MultiFile-intercepted" id="upload" enctype="multipart/form-data">
					...
					<div class="box-body">
						<div class="row">
							...
							<div class="col-xs-2">
								...
								<option value="1">Approved Restaurant</option>
									<option value="0">Hide Restaurant</option>
								</select>
							</div>
							<div class="col-xs-1">
		...
		<div class="col-xs-12">
			<div class="box">
				<div class="box-header">
					<h3 class="box-title">Resturant Pending </h3>
				...
				<div class="box-body table-responsive no-padding">
					<table class="table table-bordered">
						<tbody>
							...
							<form  action="{{url('Perform_Restaurant_Featured_And_Hide')}}" method="post" id="upload" enctype="multipart/form-data" style="margin:0px;padding: 0px; ">
								...
								<tr>
									...
									<td>{{$h->restaurant_name}}</td>
									<td>
									@foreach(App\Models\Restaurant\restaurant_type_name::selected_types_name($h->id) as $types_name )
									
										<span class="badge bg-red">{{$types_name->restaurant_type_names}}</span>
									@endforeach
									</td>

									<td>{{$h->restaurant_email}}</td>
									<td>{{$h->contact}}</td>
									<td>{{$h->city}}</td>
										<td>
										<?php  
									...
									<?php 
									if($h->detail_check==NULL||$h->photo_check==NULL){
										?>
										<td>
						...
						<hr>
							...
							<label>
								<input type="radio" name="make_hide" class="minimal" value="4">
								Hide
							</label>
							<label>
								<input type="radio" name="make_hide" class="minimal" checked value="1">
								Show
							</label>
							<label>
								<input type="radio" name="make_featured" class="minimal"  value="YES">
								Make Featured
							</label>
							<label>
								<input type="radio" name="make_featured" class="minimal"  value="No">
								Stop Featured
							</label>
							<br>
