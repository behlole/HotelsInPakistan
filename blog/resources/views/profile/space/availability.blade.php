@extends('useradmin.master')


@section('userdash')

<div class="col-md-9">
    <div class="user-form-settings">
        <div class="breadcrumb-page-bar" aria-label="breadcrumb">
            <ul class="page-breadcrumb">
                <li class="">
                    <a href="http://localhost:8001"><i class="fa fa-home"></i> Home</a>
                    <i class="fa fa-angle-right"></i>
                </li>
                <li class=" ">
                    <a href="http://localhost:8001/user/space">Spaces</a>
                    <i class="fa fa-angle-right"></i>
                </li>
                <li class=" active">
                    Availability
                </li>
            </ul>
            <div class="bravo-more-menu-user">
                <i class="icofont-settings"></i>
            </div>
        </div>
        <h2 class="title-bar no-border-bottom">
            Availability Spaces
        </h2>
        <div class="language-navigation">
            <div class="panel-body">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        <form method="get" action="" class="filter-form filter-form-left d-flex flex-column flex-sm-row"
                            role="search">
                            <input type="text" name="s" value="" placeholder="Search by name"
                                class="form-control">&nbsp;&nbsp;
                            <button class="btn-info btn btn-icon btn_search btn-sm" type="submit">Search</button>
                        </form>
                    </div>
                    <div class="col-right">
                        <span class="count-string">Showing 1 - 11 of 11 spaces</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="user-panel">
            <div class="panel-title"><strong>Availability</strong></div>
            <div class="panel-body no-padding" style="background: #f4f6f8;padding: 0px 15px;">
                <div class="row">
                    <div class="col-md-3" style="border-right: 1px solid #dee2e6;">
                        <ul class="nav nav-tabs  flex-column vertical-nav" id="items_tab" role="tablist">
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="11" data-toggle="tab" href="#calendar-11"
                                    title="STAY GREENWICH VILLAGE">#11 - STAY GREENWICH VILLAGE</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="10" data-toggle="tab" href="#calendar-10"
                                    title="LILY DALE VILLAGE">#10 - LILY DALE VILLAGE</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="9" data-toggle="tab" href="#calendar-9"
                                    title="LUXURY SINGLE">#9 - LUXURY SINGLE</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="8" data-toggle="tab" href="#calendar-8"
                                    title="PARIS GREENWICH VILLA">#8 - PARIS GREENWICH VILLA</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link active" data-id="7" data-toggle="tab" href="#calendar-7"
                                    title="EAST VILLAGE">#7 - EAST VILLAGE</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="6" data-toggle="tab" href="#calendar-6"
                                    title="THE MEATPACKING SUITES">#6 - THE MEATPACKING SUITES</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="5" data-toggle="tab" href="#calendar-5"
                                    title="DUPLEX GREENWICH">#5 - DUPLEX GREENWICH</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="4" data-toggle="tab" href="#calendar-4"
                                    title="BEST OF WEST VILLAGE">#4 - BEST OF WEST VILLAGE</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="3" data-toggle="tab" href="#calendar-3"
                                    title="BEAUTIFUL LOFT">#3 - BEAUTIFUL LOFT</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="2" data-toggle="tab" href="#calendar-2"
                                    title="LUXURY APARTMENT">#2 - LUXURY APARTMENT</a>
                            </li>
                            <li class="nav-item event-name ">
                                <a class="nav-link" data-id="1" data-toggle="tab" href="#calendar-1"
                                    title="LUXURY STUDIO">#1 - LUXURY STUDIO</a>
                            </li>
                        </ul>
                    </div>
                    <div class="col-md-9" style="background: white;padding: 15px;">
                        <div id="dates-calendar" class="dates-calendar fc fc-ltr fc-unthemed" style="">
                            <div class="fc-toolbar fc-header-toolbar">
                                <div class="fc-left">
                                    <h2>October 2020</h2>
                                </div>
                                <div class="fc-center"></div>
                                <div class="fc-right"><button type="button"
                                        class="fc-today-button fc-button fc-button-primary" disabled="">today</button>
                                    <div class="fc-button-group"><button type="button"
                                            class="fc-prev-button fc-button fc-button-primary" aria-label="prev"><span
                                                class="fc-icon fc-icon-chevron-left"></span></button><button
                                            type="button" class="fc-next-button fc-button fc-button-primary"
                                            aria-label="next"><span
                                                class="fc-icon fc-icon-chevron-right"></span></button></div>
                                </div>
                            </div>
                            <div class="fc-view-container">
                                <div class="fc-view fc-dayGridMonth-view fc-dayGrid-view" style="">
                                    <table class="">
                                        <thead class="fc-head">
                                            <tr>
                                                <td class="fc-head-container fc-widget-header">
                                                    <div class="fc-row fc-widget-header">
                                                        <table class="">
                                                            <thead>
                                                                <tr>
                                                                    <th class="fc-day-header fc-widget-header fc-mon">
                                                                        <span>Mon</span></th>
                                                                    <th class="fc-day-header fc-widget-header fc-tue">
                                                                        <span>Tue</span></th>
                                                                    <th class="fc-day-header fc-widget-header fc-wed">
                                                                        <span>Wed</span></th>
                                                                    <th class="fc-day-header fc-widget-header fc-thu">
                                                                        <span>Thu</span></th>
                                                                    <th class="fc-day-header fc-widget-header fc-fri">
                                                                        <span>Fri</span></th>
                                                                    <th class="fc-day-header fc-widget-header fc-sat">
                                                                        <span>Sat</span></th>
                                                                    <th class="fc-day-header fc-widget-header fc-sun">
                                                                        <span>Sun</span></th>
                                                                </tr>
                                                            </thead>
                                                        </table>
                                                    </div>
                                                </td>
                                            </tr>
                                        </thead>
                                        <tbody class="fc-body">
                                            <tr>
                                                <td class="fc-widget-content">
                                                    <div class="fc-scroller fc-day-grid-container"
                                                        style="overflow: hidden; height: 637px;">
                                                        <div class="fc-day-grid">
                                                            <div class="fc-row fc-week fc-widget-content fc-rigid"
                                                                style="height: 106px;">
                                                                <div class="fc-bg">
                                                                    <table class="">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-day fc-widget-content fc-mon fc-other-month fc-past"
                                                                                    data-date="2020-09-28"></td>
                                                                                <td class="fc-day fc-widget-content fc-tue fc-other-month fc-past"
                                                                                    data-date="2020-09-29"></td>
                                                                                <td class="fc-day fc-widget-content fc-wed fc-other-month fc-past"
                                                                                    data-date="2020-09-30"></td>
                                                                                <td class="fc-day fc-widget-content fc-thu fc-past"
                                                                                    data-date="2020-10-01"></td>
                                                                                <td class="fc-day fc-widget-content fc-fri fc-past"
                                                                                    data-date="2020-10-02"></td>
                                                                                <td class="fc-day fc-widget-content fc-sat fc-past"
                                                                                    data-date="2020-10-03"></td>
                                                                                <td class="fc-day fc-widget-content fc-sun fc-past"
                                                                                    data-date="2020-10-04"></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                                <div class="fc-content-skeleton">
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                                <td class="fc-day-top fc-mon fc-other-month fc-past"
                                                                                    data-date="2020-09-28"><span
                                                                                        class="fc-day-number">28</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-tue fc-other-month fc-past"
                                                                                    data-date="2020-09-29"><span
                                                                                        class="fc-day-number">29</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-wed fc-other-month fc-past"
                                                                                    data-date="2020-09-30"><span
                                                                                        class="fc-day-number">30</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-thu fc-past"
                                                                                    data-date="2020-10-01"><span
                                                                                        class="fc-day-number">1</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-fri fc-past"
                                                                                    data-date="2020-10-02"><span
                                                                                        class="fc-day-number">2</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sat fc-past"
                                                                                    data-date="2020-10-03"><span
                                                                                        class="fc-day-number">3</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sun fc-past"
                                                                                    data-date="2020-10-04"><span
                                                                                        class="fc-day-number">4</span>
                                                                                </td>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                            </div>
                                                            <div class="fc-row fc-week fc-widget-content fc-rigid"
                                                                style="height: 106px;">
                                                                <div class="fc-bg">
                                                                    <table class="">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-day fc-widget-content fc-mon fc-past"
                                                                                    data-date="2020-10-05"></td>
                                                                                <td class="fc-day fc-widget-content fc-tue fc-past"
                                                                                    data-date="2020-10-06"></td>
                                                                                <td class="fc-day fc-widget-content fc-wed fc-past"
                                                                                    data-date="2020-10-07"></td>
                                                                                <td class="fc-day fc-widget-content fc-thu fc-past"
                                                                                    data-date="2020-10-08"></td>
                                                                                <td class="fc-day fc-widget-content fc-fri fc-past"
                                                                                    data-date="2020-10-09"></td>
                                                                                <td class="fc-day fc-widget-content fc-sat fc-past"
                                                                                    data-date="2020-10-10"></td>
                                                                                <td class="fc-day fc-widget-content fc-sun fc-past"
                                                                                    data-date="2020-10-11"></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                                <div class="fc-content-skeleton">
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                                <td class="fc-day-top fc-mon fc-past"
                                                                                    data-date="2020-10-05"><span
                                                                                        class="fc-day-number">5</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-tue fc-past"
                                                                                    data-date="2020-10-06"><span
                                                                                        class="fc-day-number">6</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-wed fc-past"
                                                                                    data-date="2020-10-07"><span
                                                                                        class="fc-day-number">7</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-thu fc-past"
                                                                                    data-date="2020-10-08"><span
                                                                                        class="fc-day-number">8</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-fri fc-past"
                                                                                    data-date="2020-10-09"><span
                                                                                        class="fc-day-number">9</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sat fc-past"
                                                                                    data-date="2020-10-10"><span
                                                                                        class="fc-day-number">10</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sun fc-past"
                                                                                    data-date="2020-10-11"><span
                                                                                        class="fc-day-number">11</span>
                                                                                </td>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                            </div>
                                                            <div class="fc-row fc-week fc-widget-content fc-rigid"
                                                                style="height: 106px;">
                                                                <div class="fc-bg">
                                                                    <table class="">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-day fc-widget-content fc-mon fc-past"
                                                                                    data-date="2020-10-12"></td>
                                                                                <td class="fc-day fc-widget-content fc-tue fc-past"
                                                                                    data-date="2020-10-13"></td>
                                                                                <td class="fc-day fc-widget-content fc-wed fc-past"
                                                                                    data-date="2020-10-14"></td>
                                                                                <td class="fc-day fc-widget-content fc-thu fc-past"
                                                                                    data-date="2020-10-15"></td>
                                                                                <td class="fc-day fc-widget-content fc-fri fc-past"
                                                                                    data-date="2020-10-16"></td>
                                                                                <td class="fc-day fc-widget-content fc-sat fc-past"
                                                                                    data-date="2020-10-17"></td>
                                                                                <td class="fc-day fc-widget-content fc-sun fc-past"
                                                                                    data-date="2020-10-18"></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                                <div class="fc-content-skeleton">
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                                <td class="fc-day-top fc-mon fc-past"
                                                                                    data-date="2020-10-12"><span
                                                                                        class="fc-day-number">12</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-tue fc-past"
                                                                                    data-date="2020-10-13"><span
                                                                                        class="fc-day-number">13</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-wed fc-past"
                                                                                    data-date="2020-10-14"><span
                                                                                        class="fc-day-number">14</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-thu fc-past"
                                                                                    data-date="2020-10-15"><span
                                                                                        class="fc-day-number">15</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-fri fc-past"
                                                                                    data-date="2020-10-16"><span
                                                                                        class="fc-day-number">16</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sat fc-past"
                                                                                    data-date="2020-10-17"><span
                                                                                        class="fc-day-number">17</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sun fc-past"
                                                                                    data-date="2020-10-18"><span
                                                                                        class="fc-day-number">18</span>
                                                                                </td>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                            </div>
                                                            <div class="fc-row fc-week fc-widget-content fc-rigid"
                                                                style="height: 106px;">
                                                                <div class="fc-bg">
                                                                    <table class="">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-day fc-widget-content fc-mon fc-past"
                                                                                    data-date="2020-10-19"></td>
                                                                                <td class="fc-day fc-widget-content fc-tue fc-today "
                                                                                    data-date="2020-10-20"></td>
                                                                                <td class="fc-day fc-widget-content fc-wed fc-future"
                                                                                    data-date="2020-10-21"></td>
                                                                                <td class="fc-day fc-widget-content fc-thu fc-future"
                                                                                    data-date="2020-10-22"></td>
                                                                                <td class="fc-day fc-widget-content fc-fri fc-future"
                                                                                    data-date="2020-10-23"></td>
                                                                                <td class="fc-day fc-widget-content fc-sat fc-future"
                                                                                    data-date="2020-10-24"></td>
                                                                                <td class="fc-day fc-widget-content fc-sun fc-future"
                                                                                    data-date="2020-10-25"></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                                <div class="fc-content-skeleton">
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                                <td class="fc-day-top fc-mon fc-past"
                                                                                    data-date="2020-10-19"><span
                                                                                        class="fc-day-number">19</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-tue fc-today "
                                                                                    data-date="2020-10-20"><span
                                                                                        class="fc-day-number">20</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-wed fc-future"
                                                                                    data-date="2020-10-21"><span
                                                                                        class="fc-day-number">21</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-thu fc-future"
                                                                                    data-date="2020-10-22"><span
                                                                                        class="fc-day-number">22</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-fri fc-future"
                                                                                    data-date="2020-10-23"><span
                                                                                        class="fc-day-number">23</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sat fc-future"
                                                                                    data-date="2020-10-24"><span
                                                                                        class="fc-day-number">24</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sun fc-future"
                                                                                    data-date="2020-10-25"><span
                                                                                        class="fc-day-number">25</span>
                                                                                </td>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                            </div>
                                                            <div class="fc-row fc-week fc-widget-content fc-rigid"
                                                                style="height: 106px;">
                                                                <div class="fc-bg">
                                                                    <table class="">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-day fc-widget-content fc-mon fc-future"
                                                                                    data-date="2020-10-26"></td>
                                                                                <td class="fc-day fc-widget-content fc-tue fc-future"
                                                                                    data-date="2020-10-27"></td>
                                                                                <td class="fc-day fc-widget-content fc-wed fc-future"
                                                                                    data-date="2020-10-28"></td>
                                                                                <td class="fc-day fc-widget-content fc-thu fc-future"
                                                                                    data-date="2020-10-29"></td>
                                                                                <td class="fc-day fc-widget-content fc-fri fc-future"
                                                                                    data-date="2020-10-30"></td>
                                                                                <td class="fc-day fc-widget-content fc-sat fc-future"
                                                                                    data-date="2020-10-31"></td>
                                                                                <td class="fc-day fc-widget-content fc-sun fc-other-month fc-future"
                                                                                    data-date="2020-11-01"></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                                <div class="fc-content-skeleton">
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                                <td class="fc-day-top fc-mon fc-future"
                                                                                    data-date="2020-10-26"><span
                                                                                        class="fc-day-number">26</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-tue fc-future"
                                                                                    data-date="2020-10-27"><span
                                                                                        class="fc-day-number">27</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-wed fc-future"
                                                                                    data-date="2020-10-28"><span
                                                                                        class="fc-day-number">28</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-thu fc-future"
                                                                                    data-date="2020-10-29"><span
                                                                                        class="fc-day-number">29</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-fri fc-future"
                                                                                    data-date="2020-10-30"><span
                                                                                        class="fc-day-number">30</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sat fc-future"
                                                                                    data-date="2020-10-31"><span
                                                                                        class="fc-day-number">31</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sun fc-other-month fc-future"
                                                                                    data-date="2020-11-01"><span
                                                                                        class="fc-day-number">1</span>
                                                                                </td>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                            </div>
                                                            <div class="fc-row fc-week fc-widget-content fc-rigid"
                                                                style="height: 107px;">
                                                                <div class="fc-bg">
                                                                    <table class="">
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-day fc-widget-content fc-mon fc-other-month fc-future"
                                                                                    data-date="2020-11-02"></td>
                                                                                <td class="fc-day fc-widget-content fc-tue fc-other-month fc-future"
                                                                                    data-date="2020-11-03"></td>
                                                                                <td class="fc-day fc-widget-content fc-wed fc-other-month fc-future"
                                                                                    data-date="2020-11-04"></td>
                                                                                <td class="fc-day fc-widget-content fc-thu fc-other-month fc-future"
                                                                                    data-date="2020-11-05"></td>
                                                                                <td class="fc-day fc-widget-content fc-fri fc-other-month fc-future"
                                                                                    data-date="2020-11-06"></td>
                                                                                <td class="fc-day fc-widget-content fc-sat fc-other-month fc-future"
                                                                                    data-date="2020-11-07"></td>
                                                                                <td class="fc-day fc-widget-content fc-sun fc-other-month fc-future"
                                                                                    data-date="2020-11-08"></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                                <div class="fc-content-skeleton">
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                                <td class="fc-day-top fc-mon fc-other-month fc-future"
                                                                                    data-date="2020-11-02"><span
                                                                                        class="fc-day-number">2</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-tue fc-other-month fc-future"
                                                                                    data-date="2020-11-03"><span
                                                                                        class="fc-day-number">3</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-wed fc-other-month fc-future"
                                                                                    data-date="2020-11-04"><span
                                                                                        class="fc-day-number">4</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-thu fc-other-month fc-future"
                                                                                    data-date="2020-11-05"><span
                                                                                        class="fc-day-number">5</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-fri fc-other-month fc-future"
                                                                                    data-date="2020-11-06"><span
                                                                                        class="fc-day-number">6</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sat fc-other-month fc-future"
                                                                                    data-date="2020-11-07"><span
                                                                                        class="fc-day-number">7</span>
                                                                                </td>
                                                                                <td class="fc-day-top fc-sun fc-other-month fc-future"
                                                                                    data-date="2020-11-08"><span
                                                                                        class="fc-day-number">8</span>
                                                                                </td>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody>
                                                                            <tr>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                                <td class="fc-event-container"><a
                                                                                        class="fc-day-grid-event fc-h-event fc-event fc-start fc-end"
                                                                                        style="color:#2791fe">
                                                                                        <div class="fc-content"> <span
                                                                                                class="fc-title">$260</span>
                                                                                        </div>
                                                                                    </a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="d-flex justify-content-center">

        </div>
        <div id="bravo_modal_calendar" class="modal fade">
            <div role="document" class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Date Information</h5> <button type="button" data-dismiss="modal"
                            aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                    </div>
                    <div class="modal-body">
                        <form novalidate="novalidate" onsubmit="return false"
                            class="row form_modal_calendar form-horizontal">
                            <div class="col-md-6">
                                <div class="form-group"><label>Date Ranges</label> <input readonly="readonly"
                                        type="text" class="form-control has-daterangepicker"></div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group"><label>Status</label> <br> <label><input true-value="1"
                                            false-value="0" type="checkbox"> Available for booking?</label></div>
                            </div>
                            <div class="col-md-6" style="display: none;">
                                <div class="form-group"><label>Price</label> <input type="number" class="form-control">
                                </div>
                            </div>
                            <div class="col-md-6 d-none" style="display: none;">
                                <div class="form-group"><label>Instant Booking?</label> <br> <label><input
                                            true-value="1" false-value="0" type="checkbox"> Enable instant
                                        booking</label></div>
                            </div>
                        </form>
                        <!---->
                    </div>
                    <div class="modal-footer"><button type="button" data-dismiss="modal"
                            class="btn btn-secondary">Close</button> <button type="button" class="btn btn-primary">Save
                            changes</button></div>
                </div>
            </div>
        </div>
    </div>
</div>

@endsection
