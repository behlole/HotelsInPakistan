...
class AdminHotelController extends Controller
{
	...
	function Hotel_Pending(){
		...
		->get();
		return view('Admin/Hotel/hotel_pending',compact('data'));


  //return view('admindashpanel/hoteldash/hotel_pending',compact('data','sub_facility'));
	}

	function search_hotel_by_pending(Request $request){
	...
	function Perform_Hotel_Pending_Approval(Request $request){
	...
	function Hotel_Approved(){
		$data=Hotel::join('city_names','hotels.city','=','city_names.id')
		->whereBetween('hotels.hotel_status', array(1,4))
		->where('hotels.propcheck','=', 1)
		->select('hotels.*','city_names.city_name as city')
		->get();
		return view('Admin/Hotel/Hotel_Approved',compact('data'));

	}

	function Perform_Hotel_Featured_And_Hide(Request $request){
		...
		return redirect('/Hotel_Approved')->with('status_ok', 'Hotel Status Updated');
	}


	function Search_Hotel_Approved_or_Hide(Request $request){
		$from= date('y-01-01');
		$to = date('y-m-d');
		if($request->from){
			$from=$request->from;
  ...
  //Hotel_Premiume_Listing_By_City					
	...
	function Hotel_Premiume_Listing_By_City(){
		...
		->get();
			...
			$premium_listings=PremiumListing::join('hotels','premium_listings.prop_id','=','hotels.id')->where('hotels.propcheck','=', 1)->join('city_names','premium_listings.city_id','=','city_names.id')->select('city_names.*','city_names.city_name as city',
			DB::raw("COUNT(city_names.id) as total_listing"))->groupBy('city_names.id')->get();


		return view('Admin/Hotel/Hotel_Premiume',compact('data','premium_listings'));

	}
	function Search_Hotel_Premiume(Request $request){
	...
	function Perform_Premiume_By_City(Request $request){
		$this->validate($request, [
			'check_list' => 'required',
			'city'  => 'required'
		]);
		...
		return redirect('/Hotel_Premiume_Listing_By_City')->with('status_ok', 'Hotel Status Updated');
